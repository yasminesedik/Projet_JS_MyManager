            ...
            <div class="games-view">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    ...
                    <button class="btn btn-primary" id="add-game-btn">
                        <i class="fas fa-plus"></i> <span data-i18n="games.add">${i18n.t('games.add')}</span>
                    </button>
                </div>
    ...
    renderTable() {
        ...
        this.table = new Table(container, {
            columns: [
                ...
                { key: 'price', label: i18n.t('games.price'), sortable: true, type: 'currency' },
                { key: 'releaseDate', label: i18n.t('games.releaseDate'), sortable: true, type: 'date' },
    ...
    async viewGame(id) {
        ...
        const modal = new Modal({
            ...
            body: `
                <div class="row">
                    <div class="col-md-6">
                        <p><strong>${i18n.t('games.genre')}:</strong> ${game.genre}</p>
                        <p><strong>${i18n.t('games.platform')}:</strong> ${game.platform}</p>
                        <p><strong>${i18n.t('games.price')}:</strong> $${game.price}</p>
                        <p><strong>${i18n.t('games.releaseDate')}:</strong> ${new Date(game.releaseDate).toLocaleDateString()}</p>
                        <p><strong>${i18n.t('games.rating')}:</strong> ${game.rating}/10</p>
                    </div>
                    <div class="col-md-6">
        ...
        modalEl.querySelector('#export-pdf-btn').addEventListener('click', () => {
            ExportUtils.exportToPDF(game, game.name, `${game.name.replace(/\s+/g, '_')}.pdf`);
        });
    }
    ...
    showGameForm(game = null) {
        ...
        const modal = new Modal({
            ...
            body: `
                <form id="game-form">
                    <div class="mb-3">
                        <label class="form-label">${i18n.t('games.name')}</label>
                        <input type="text" class="form-control" id="game-name" value="${game?.name || ''}" required>
                    </div>
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <label class="form-label">${i18n.t('games.genre')}</label>
                            <select class="form-select" id="game-genre" required>
                                <option value="">${i18n.t('common.filter')}...</option>
                                ${this.genres.map(g => `
                                    <option value="${g.name}" ${game?.genre === g.name ? 'selected' : ''}>${g.name}</option>
                                `).join('')}
                            </select>
                        </div>
                        <div class="col-md-6 mb-3">
                            <label class="form-label">${i18n.t('games.platform')}</label>
                    ...
                    <div class="row">
                        <div class="col-md-4 mb-3">
                            <label class="form-label">${i18n.t('games.price')}</label>
                            <input type="number" step="0.01" class="form-control" id="game-price" value="${game?.price || ''}" required>
                        </div>
                        <div class="col-md-4 mb-3">
                            <label class="form-label">${i18n.t('games.releaseDate')}</label>
                            <input type="date" class="form-control" id="game-release-date" value="${game?.releaseDate || ''}" required>
                        </div>
                        <div class="col-md-4 mb-3">
                            <label class="form-label">${i18n.t('games.rating')}</label>
                            <input type="number" step="0.1" min="0" max="10" class="form-control" id="game-rating" value="${game?.rating || ''}" required>
                        </div>
                    </div>
                    <div class="mb-3">
                        <label class="form-label">${i18n.t('games.description')}</label>
                        <textarea class="form-control" id="game-description" rows="3">${game?.description || ''}</textarea>
                    </div>
                </form>
            `,
            footer: `
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">${i18n.t('common.cancel')}</button>
                <button type="button" class="btn btn-primary" id="save-game-btn">${i18n.t('common.save')}</button>
            `
        });
