import { generateFakePlayers } from "./fakerPlayers.js";
...
export class PlayerService {
  ...
  initData() {
    if (!localStorage.getItem(this.storageKey)) {
      const players = generateFakePlayers(60);
  ...
  getById(id) {
    ...
    return Promise.resolve(players.find((p) => p.id === +id) || null);
  }
  ...
  update(id, player) {
    ...
    if (index === -1) return Promise.reject("Player not found");

    players[index] = { ...player, id: +id };
    localStorage.setItem(this.storageKey, JSON.stringify(players));
  ...
  delete(id) {
    ...
    localStorage.setItem(
      this.storageKey,
      JSON.stringify(players.filter((p) => p.id !== +id))
    );
